...
use pytauri_moviepy_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m pytauri_moviepy`,
    // i.e, run the `src-tauri/python/pytauri_moviepy/__main__.py`
    let py_script = PythonScript::Module("pytauri_moviepy".into());
